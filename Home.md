...
5. Обрати внимание так-же на [облако от Алишера Усманова](https://cloud.mail.ru/) и [облако хуяндекса](https://disk.yandex.ru/) с вероятностью 99% их не заблочат т.к. сервера находятся у нас, поэтому можешь использовать их методом: создал акк с мультипочтой - насоздавал сотни акков - разбил винраром на маленькие запароленные архивы архив с трукриптовским контейнером, не забудь продублировать схему на 2 мультиакк, но не забывай о том что облаку нельзя доверять критически важную инфу ведь вполне вероятно что модераторы потрут твои запароленные архивы из-за новой директивы ФСБ.
